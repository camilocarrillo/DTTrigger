...
//------------------------------------------------------------------
MuonPath::MuonPath(DTPrimitive *ptrPrimitive[4]) throw (CException) {
  LOGTRACE("Creando un 'MuonPath'");
  ...
  for (int i = 0; i <= 3; i++) {
    if ( (prim[i] = ptrPrimitive[i]) == NULL )
      throw CException("Unable to create 'MuonPath'. Null 'Primitive'.");
  }
...
MuonPath::~MuonPath() {
  LOGTRACE("Destruyendo un 'MuonPath'");
 ...
 */
void MuonPath::setPrimitive(DTPrimitive *ptr, int layer) throw (CException) {
  if (ptr == NULL) throw CException("NULL 'Primitive'.");
...
bool MuonPath::isEqualTo(MuonPath *ptr) {
  ...
  for (int i = 0; i <= 3; i++) {
    ...
    if (this->getPrimitive(i)->isValidTime() && 
        ptr->getPrimitive(i)->isValidTime() )
    {
      if (ptr->getPrimitive(i)->getSuperLayerId() !=
          this->getPrimitive(i)->getSuperLayerId() ||
        
          ptr->getPrimitive(i)->getChannelId() !=
          this->getPrimitive(i)->getChannelId()    ||

          ptr->getPrimitive(i)->getTDCTime() !=
          this->getPrimitive(i)->getTDCTime()      ||
           ...
           */
          ptr->getPrimitive(i)->getOrbit() !=
          this->getPrimitive(i)->getOrbit()        ||

          (ptr->getLateralComb())[i] != 
          (this->getLateralComb())[i]
         ) 
        return false;
    }
    else {
