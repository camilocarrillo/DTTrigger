 ...
 * File name:  multichdatamixer.cpp
 * Language:   C++
 *
 * *********************************************************************
 * Description:
 ...
 *
 * *********************************************************************
 * Copyright (c) 2015-04-16 Jose Manuel Cela <josemanuel.cela@ciemat.es>
 *
...
{
  LOGTRACE("Creando un 'MultiChDataMixer'");
  setRRobin(false);
...
MultiChDataMixer::~MultiChDataMixer() {
  LOGTRACE("Destruyendo un 'MultiChDataMixer'");
}
...
void MultiChDataMixer::mixChannels(int supLayer, int pathId) {
  ...
  */
  ...
  LOGDEBUG_INT("'MultiChDataMixer::mixChannels' mezcla completa: ", pathId);
    ...
    for (chIdx[1] = 0; chIdx[1] < numPrimsPerLayer[1]; chIdx[1]++)
      for (chIdx[2] = 0; chIdx[2] < numPrimsPerLayer[2]; chIdx[2]++)
        for (chIdx[3] = 0; chIdx[3] < numPrimsPerLayer[3]; chIdx[3]++) {
          ...
          */
          LOGTRACE("'MultiChDataMixer::mixChannels'. Combinando primitivas.");

          for (int i = 0; i <= 3; i++) {
            ptrPrimitive[i] = new DTPrimitive( (data[i])[chIdx[i]] );

            LOGDEBUG(
              NString("Capa: ").nAppend("%i", ptrPrimitive[i]->getLayerId()).\
                     sAppend(" Canal: ").\
                     nAppend("%i", ptrPrimitive[i]->getChannelId()).\
                     sAppend(" TDCTime: ").
                     nAppend("%i", ptrPrimitive[i]->getTDCTime()));
          }
          ...
          if (ptrMuonPath->isAnalyzable()) {
            ...
            */
            if ( !isEqualComb2Previous(ptrPrimitive) ) {
              LOGTRACE("'MultiChDataMixer::mixChannels'. Pushing 'MuonPath'");

              /* Este código es una chapuza para depuraciones */
              LOGINFO(
                NString("'MuonPath' analyzable. TDC Time's: ").\
                  nAppend("%i",
                    ptrMuonPath->getPrimitive(0)->getTDCTime()).sAppend(" ").\
                  nAppend("%i",
                    ptrMuonPath->getPrimitive(1)->getTDCTime()).sAppend(" ").\
                  nAppend("%i",
                    ptrMuonPath->getPrimitive(2)->getTDCTime()).sAppend(" ").\
                  nAppend("%i",
                    ptrMuonPath->getPrimitive(3)->getTDCTime()).sAppend(" "));
              /*
              ...
              */
              ...
              LOGTRACE("'MultiChDataMixer::mixChannels'. Pushed 'MuonPath'");
            }
            else delete ptrMuonPath;
          }
          else {
            LOGNOTICE(
              NString("PRIM not analyzable. TDC Time's: ").\
                nAppend("%i",
                  ptrMuonPath->getPrimitive(0)->getTDCTime()).sAppend(" ").\
                nAppend("%i",
                  ptrMuonPath->getPrimitive(1)->getTDCTime()).sAppend(" ").\
...
void MultiChDataMixer::setChCurrentTime(int time) {
  ...
  LOGTRACEFN_IN;
  LOGDEBUG_INT("Cambiando CHANNEL current Time: ", time);
