...
PathAnalyzer::~PathAnalyzer() {
  std::cout<<"Destruyendo un 'PathAnalyzer'"<<std::endl;
}
...
void PathAnalyzer::evaluatePathQuality(MuonPath *mPath) {
  ...
  std::cout<<"Evaluando PathQ. Celda base: "<<mPath->getBaseChannelId()<<std::endl;
  std::cout<<"Total lateralidades: "<<totalNumValLateralities<<std::endl;
                                          ...
                                          LATQ_TYPE *latQuality)
{
  ...
  */
  if ( !sameBXValue(latQResult) ) {
    ...
    std::cout<<"Lateralidad DESCARTADA. Tolerancia de BX excedida"<<std::endl;
    return;
  }
...
//     latQuality->bxValue = latQResult[0].bxValue;
    ...
    std::cout<<"Lateralidad ACEPTADA. HIGHQ."<<std::endl;
  }
  // Sólo una traza disjunta válida => Traza de muón incompleta pero válida.
        ...
        latQResult[2].latQValid || latQResult[3].latQValid)
    {
      ...
      std::cout<<"Lateralidad ACEPTADA. LOWQ."<<std::endl;
    }
    else {
      std::cout<<"Lateralidad DESCARTADA. NOPATH."<<std::endl;
    }
  }
}
                            ...
                            MuonPath* mPath, PARTIAL_LATQ_TYPE *latq)
{
  ...
  std::cout<<"Iniciando validacion de MuonPath para capas: "
	   <<layerIndex[0]<<"/"
	   <<layerIndex[1]<<"/"
	   <<layerIndex[2]<<std::endl;
  
  std::cout<<"Lateralidades parciales: "
	   <<sideComb[0]<<"/"
	   <<sideComb[1]<<"/"
	   <<sideComb[2]<<std::endl;
  ...
  if (validCells != 3) {
      std::cout<<"No hay 3 celdas validas."<<std::endl;
      return;
  }
  ...
  if(denominator == 0) {
      std::cout<<"Imposible calcular BX. Denominador para BX = 0."<<std::endl;
      return;
  }
  ...
  if(bxValue < 0) {
      std::cout<<"Combinacion no valida. BX Negativo."<<std::endl;
    return;
  }
  ...
  for (int i = 0; i < 3; i++)
    if (mPath->getPrimitive(layerIndex[i])->isValidTime()) {
	  ...
	  std::cout<<"Valor de BX inválido. Al menos un tiempo de TDC sin sentido"<<std::endl;
        return;
      }
    }
                             ...
                             MuonPath* mPath, int bxValue)
{
  ...
  std::cout<<"EQTerm(Main): "<<eqTerm<<std::endl;
